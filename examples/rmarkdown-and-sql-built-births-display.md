...
``` r
ggplot(my_birth_data, aes(x = year, y = total_births)) + geom_point() + labs(x = "YEAR", y = "TOTAL BIRHTS") + scale_x_continuous(name = "YEAR", breaks = my_birth_data$year, labels = as.character(my_birth_data$year))
```
